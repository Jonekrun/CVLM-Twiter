...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", **kwargs):
    ...
    if "fka" in model_path.lower() or "cvlm-llava" in model_path.lower():
        ...
        llava_config = KnowledgeLlavaConfig.from_pretrained("CVLM/liuhaotian/llava-v1.5-7b")
        model =  KnowledgeOPTQformerLlavaLlamaForCausalLM.from_pretrained(model_base, torch_dtype=torch.float16, low_cpu_mem_usage=True,
                                                          device_map="auto", config=llava_config)
        print("SAM ...")
        # print(model_path)
        ...
        model = KnowledgeOPTLlavaLlamaForCausalLM.from_pretrained(model_base, torch_dtype=torch.float16, low_cpu_mem_usage=True,
                                                          device_map="auto", config=llava_config)
        print(model_path)
        if os.path.exists(os.path.join(model_path, 'non_lora_trainables.bin')):
            non_lora_trainables = torch.load(os.path.join(model_path, 'non_lora_trainables.bin'), map_location='cpu')
    ...
    if 'llava' in model_name.lower() or "knowledge" in model_path.lower():
        ...
        if "opt_knowledge" in model_path.lower():
